...
addBtn.onclick = ()=>{ //when user click on plus icon button
  ...
  if(getLocalStorageData == null){ //if localstorage has no data
    listArr = []; //create a blank Arr
  }else{
    listArr = JSON.parse(getLocalStorageData);  //transforming json string into a js object
  }
  listArr.push(userEnteredValue); //pushing or adding new value in Arr
...
function showTasks(){
  ...
  pendingTasksNumb.textContent = listArr.length; //passing the Arr length in pendingtask
  if(listArr.length > 0){ //if Arr length is greater than 0
